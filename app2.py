...
AGRICULTURE_KEYWORDS = [
    "crop", "farming", "soil", "irrigation", "fertilizer", "harvest",
    "plant", "pesticide", "agriculture", "yield", "weather", "climate",
    "organic", "farmer", "greenhouse", "horticulture", "livestock",
    "disease", "pest", "agronomy", "cultivation", "drought"
]

@app.route("/chat", methods=["POST"])
def chat():
    """Handle chatbot requests, responding only to agriculture-related queries."""
    data = request.get_json()
    if not data or "message" not in data:
        return jsonify({"error": "No message provided"}), 400

    user_message = data["message"].lower()  # Convert message to lowercase
    print("User message for chatbot:", user_message)  # Debug print

    # Check if the message is agriculture-related
    if not any(keyword in user_message for keyword in AGRICULTURE_KEYWORDS):
        return jsonify({"response": "I'm here to discuss agriculture topics. Please ask about farming, crops, soil, or related subjects."})

    try:
        # Generate response using Gemini API
        response = gemini_model.generate_content(user_message)

        # Ensure response is within 3-4 lines
        response_text = response.text.strip()
        lines = response_text.split("\n")
        short_response = " ".join(lines[:6]) if len(lines) > 4 else response_text

        print("Gemini response:", short_response)  # Debug print
